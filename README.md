...
- **AI 大模型接入**：配置 OpenAI 兼容接口（DeepSeek/豆包/千问/文心等），支持 AI 视频文案专家、AI 数据分析专家，每日额度管理（chat/review 独立计数，分别 20 次/5 次上限；取消/失败自动退还）
...
- **服务端权威管理**：额度在 AI 代理接口内校验和扣减，绕过页面直接调用也无法超限；请求失败、超时或用户中途取消会自动退还，不白扣次数
- AI 视频文案（chat）每日上限 20 次，可用环境变量 `MCB_LLM_DAILY_LIMIT` 调整；AI 数据分析（review）每日上限 5 次，可用 `MCB_REVIEW_DAILY_LIMIT` 调整（两类独立计数，按登录用户分别计）
- 跨天自动重置；额度文件（llmQuota.json）由服务端专管，客户端只读，无法自行清零
