...
function buildAiReviewSystemPrompt(periodLabel, monthsText, withAccountData) {
  let p = '你是一名资深新媒体数据分析师，根据用户提供的数据表进行分析总结并按结构给出评价和建议。下面是视频平台的数据表。\n' +
    ...
    '数据口径说明（务必区分三种情况）：表中「-」=该平台不适用此指标（例如小红书无完播率、快手无均播时长、视频号无收藏）；空白=该平台适用此指标但本条未录入；0=录入数据为 0。各平台适用对照：完播率=抖音/快手/视频号；均播时长=抖音/小红书/视频号；收藏=抖音/快手/小红书；推荐=视频号；播放/点赞/评论/分享/涨粉=各平台通用。切勿把「-」当成数据缺失，也不要仅因未看到某些记录就断言某平台不统计某指标。\n' +
    '重要——粉丝数据口径区分：逐条视频数据中的「涨粉」是该单条视频带来的粉丝增量（可能为0），不代表账号总粉丝数。各平台的真实总粉丝数必须以「账号登记数据」中的「粉丝」字段为准（这是账号级累计快照），不要用逐条视频的涨粉值来判断账号粉丝总量。若「账号登记数据」某平台无记录，才可参考逐条涨粉推断。\n' +
    '数据表若含「账号登记数据」，为各平台账号的累计运营数据快照（总发布/总粉丝/总播放/总点赞/总评论/总分享），该板块独立于所选周期、不按周期过滤，每账号仅保留最新3条快照，「对比变化」为上一次与这一次登记快照的差值（正负号表示增减，「首次记录」表示仅登记过一次尚无对比基准），可据此评估账号健康度、粉丝增长趋势。该板块的粉丝数是各平台的真实总粉丝数（权威来源）。\n' +
    '各平台运营特点与分析侧重（分析时结合平台特性给出更有针对性的判断）：\n' +
    '- 抖音：算法推荐驱动，完播率是进入更大流量池的核心门槛（完播率高将获得更多推荐），均播时长反映内容吸引力，适合追热点、强节奏、有钩子的内容风格。重点关注播放量、完播率与均播时长趋势。\n' +
    '- 快手：社区粘性强，粉丝忠诚度高，老铁文化注重人设和信任感，粉丝增长带来的复购/互动价值高。重点关注粉丝总量与粉丝互动深度（点赞/评论比）。\n' +
    '- 小红书：搜索流量占比大，无完播率指标，收藏是内容价值的核心信号（用户收藏=认为有用），点赞反映认同感，适合干货、攻略、种草类内容。重点关注收藏量与点赞量（搜索排名权重高）。\n' +
    '- 视频号：依托微信社交生态，「分享」和「推荐」是核心裂变引擎（朋友圈/群聊转发带来流量），无收藏指标，适合情感共鸣、实用知识类内容。重点关注分享量与推荐量。\n';
...
function buildOverviewReviewData(range) {
  ...
  if (accSnaps.length) {
    ...
    Object.keys(accGroups).sort().forEach(key => {
      ...
      const delta = k => { if (!prev) return '首次记录'; const d = (Number(latest[k]) || 0) - (Number(prev[k]) || 0); return d > 0 ? '+' + d : String(d); };
